...
- **Trust on first use, with something to trust.** A code is an address and
  ...
  refused (a host with several peers hands out one invitation per device). Candidates are heard side by side, up to a bounded number, because
...
- **A request that waits.** Sending while the link happens to be down is not
  an error: it waits for the next session, and gives up only once nothing has
  moved for `RequestDeadline` (`TransferStallTimeout` for content and files). Only a
...
- **One way to send anything, of any size.** A kilobyte of JSON and a 20 GB
  ...
  wire format is [docs/exchanges.md](exchanges.md).
...
stack outright, and content of several megabytes interrupted midway in either
direction that resumes into the handler already reading it.
...
| `src/Tailcat.Link` | (new) pair two machines once, stay linked |
...
The protocol is specified byte for byte in [docs/relay1.md](relay1.md),
